...
import { IExtrinsicRepository } from '@repositories/extrinsic/IExtrinsicRepository'
...
import { IEraRepository } from '@repositories/era/IEraRepository'
import { ILogger } from '@modules/logger/ILogger'
import { IConfigService } from '@services/config/config.types'
import { IBlocksService } from '@services/blocks'
import { IStakingService } from '@services/staking/staking.types'
...
export class WatchdogService implements IWatchdogService {
  ...
  constructor(
    ...
    @inject(IExtrinsicRepository) protected extrinsicRepository: IExtrinsicRepository,
    @inject(IEventRepository) protected eventRepository: IEventRepository,
    @inject(IEraRepository) protected eraRepository: IEraRepository,
  ...
  private async verifyBlock(blockId: number): Promise<void> {
    ...
    if (!blockFromDB) {
      ...
      await this.blocksService.processBlock(blockId, true)
      return
    }
    ...
    if (!isBlockValidResult) {
      try {
        ...
        this.logger.error(
          { error },
          `error in blocksService.processBlock invocation when try to resync missed events for block ${blockId}`
        )
      }
    }
  ...
  private async validateEraPayout(blockFromDB: IBlock): Promise<void> {
    ...
    if (!eraFromDb) {
      this.stakingService.addToQueue({ eraId: eraId?.toString(), blockHash: blockFromDB.hash })
      return
    }
  ...
  private async isStartParamsValid(startBlockId: number): Promise<boolean> {
    ...
    if (!isStartBlockValid) {
      this.logger.error(
        `Attempt to run verification with blockId ${startBlockId} greater than current watchdog_verify_height. 
        ` + `Exit with error.`
      )
      return false
    }
